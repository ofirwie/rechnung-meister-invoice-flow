...
export default function InvoiceHistoryTable({ language, onInvoiceView }: InvoiceHistoryTableProps) {
  ...
  const getStatusText = (status: InvoiceHistory['status']) => {
    switch (status) {
      case 'paid':
        return language === 'de' ? 'Bezahlt' : language === 'fr' ? 'Payé' : language === 'he' ? 'שולם' : 'Paid';
      case 'sent':
        return language === 'de' ? 'Gesendet' : language === 'fr' ? 'Envoyé' : language === 'he' ? 'נשלח' : 'Sent';
      case 'overdue':
        return language === 'de' ? 'Überfällig' : language === 'fr' ? 'En retard' : language === 'he' ? 'באיחור' : 'Overdue';
      case 'draft':
        return language === 'de' ? 'Entwurf' : language === 'fr' ? 'Brouillon' : language === 'he' ? 'טיוטה' : 'Draft';
      default:
        return status;
    }
  };
